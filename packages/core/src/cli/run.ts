...
async function runSkillsDriftCheck(skillsDir: string): Promise<void> {
  ...
  if (!interactive) {
    process.stderr.write(
      `${chalk.yellow('!')} Skills out of date (${names}). Run \`open-aippt sync:skills\` to update.\n`,
    );
    return;
  }
  ...
  try {
    ...
    if (answer === '' || answer === 'y' || answer === 'yes') {
      ...
      process.stdout.write(chalk.dim('Skipped. Run `open-aippt sync:skills` later to update.\n'));
    }
  } finally {
    rl.close();
  }
...
export async function run(argv: string[]): Promise<void> {
  ...
  program
    ...
    .showHelpAfterError(chalk.dim('(run `open-aippt --help` for usage)'));
  ...
  program
    .command('sync:skills')
    .description('Sync built-in skills from @open-aippt/core into this workspace')
