---
title: open-aippt sync:skills
description: Refresh the bundled agent skills inside an existing workspace.
---

```npm
open-aippt sync:skills
# or
npm run sync:skills
```

Copies the latest skill definitions from your installed `@open-aippt/core`
into the workspace under `.agents/skills/`, and refreshes the
`.claude/skills/` symlinks so Claude Code picks them up too.

Run this after upgrading `@open-aippt/core` — that's how you pull in skill
...
.agents/
└── skills/
    └── <name>/        # canonical copy from @open-aippt/core
.claude/
└── skills/
...
`open-aippt dev` runs the same drift check at startup. If any skill is out
...
`OPEN_AIPPT_SKIP_SKILLS_CHECK=1`) — see [open-aippt dev](/docs/cli/dev).
