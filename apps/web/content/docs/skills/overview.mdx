---
title: Overview
description: How open-aippt and your coding agent work together.
---

open-aippt treats your coding agent as a first-class user. Decks are React,
agents are great at React, the gap between *"make slides about X"* and a
polished deck is mostly the runtime — which open-aippt provides.

## What ships in the workspace

`npx @open-aippt/cli init` generates an `AGENTS.md` (and `CLAUDE.md`) at the
...
they version with the project. `open-aippt sync:skills` re-syncs them from
...
- Don't touch `package.json`, `open-aippt.config.ts`, or other slides.
...
scaffolder; edit `AGENTS.md` and re-run `open-aippt sync:skills` if you
