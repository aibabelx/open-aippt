...
export async function init(opts: InitOptions): Promise<void> {
  ...
  if (!existsSync(TEMPLATE_DIR)) {
    throw new Error(
      `Template missing at ${TEMPLATE_DIR}. If you are running from source, run \`pnpm --filter @open-aippt/cli build\` first.`,
    );
  }
  ...
  if (existsSync(pkgPath)) {
    ...
    pkg.version = '0.0.0';
    pkg.private = true;
    if (pkg.dependencies?.['@open-aippt/core']) {
      pkg.dependencies['@open-aippt/core'] = coreVersionRange();
    }
