...
import type { OpenAipptConfig } from '../config.ts';

export type { OpenAipptConfig };

export type OpenAipptPluginOptions = {
  userCwd: string;
...
export function openAipptPlugin(opts: OpenAipptPluginOptions): Plugin {
...
export async function loadUserConfig(userCwd: string): Promise<OpenAipptConfig> {
  ...
  return (loaded?.config ?? {}) as OpenAipptConfig;
}
