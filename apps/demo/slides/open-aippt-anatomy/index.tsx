import { type DesignSystem, type Page, type SlideMeta, useSlidePageNumber } from '@open-aippt/core';
  ...
  return (
    ...
    >
      <span>
        ...
        <span style={{ color: text2 }}>open-aippt</span>
      </span>
  ...
  const lines: { code: ReactNode; tag?: string; tagColor?: string }[] = [
    {
      code: (
        <>
          ...
          <Token c={violet}>from</Token> <Token c={mint}>'@open-aippt/core'</Token>
          {';'}
        </>
      ),
    },
    ...
    <div style={{ ...fill, padding: '120px 100px' }}>
      ...
      >
        ...
        >
          ...
          <div style={{ color: 'var(--osd-accent)' }}>virtual:open-aippt/slides</div>
          <div style={{ height: 14 }} />
...
const VitePluginHooks: Page = () => {
  const hooks = [
    ...
    { name: 'resolveId()', desc: 'claim the three virtual:open-aippt/* ids', tint: mint },
...
const Cli: Page = () => {
  const cmds = [
    ...
    { cmd: 'open-aippt build', desc: 'viteBuild() → static SPA in dist/', tint: mint },
    { cmd: 'open-aippt preview', desc: 'serve dist/ for local check', tint: warm },
    {
      cmd: 'npx @open-aippt/cli init',
...
export const meta: SlideMeta = { title: 'Inside open-aippt', createdAt: '2026-05-05T17:45:52Z' };
