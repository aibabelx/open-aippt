...
import { loadUserConfig, type OpenAipptConfig, openAipptPlugin } from './open-aippt-plugin.ts';
...
export async function createViteConfig(opts: CreateViteConfigOptions): Promise<InlineConfig> {
  ...
  return {
    ...
    plugins: [
      ...
      react(),
      tailwindcss(),
      openAipptPlugin({ userCwd, config, coreVersion: CORE_VERSION }),
    ...
    optimizeDeps: {
      ...
      // The app source ships inside node_modules/@open-aippt/core/src/app, so
        ...
        plugins: [
          {
            ...
            setup(build) {
              build.onResolve({ filter: /^virtual:open-aippt\// }, (args) => ({
                path: args.path,
                external: true,
              }));
            },
