...
The locale only affects the **runtime UI** that open-aippt renders around
...
All presets are re-exported from the `@open-aippt/core/locale` subpath:

```ts
import { en, zhTW, zhCN, ja } from '@open-aippt/core/locale';
```
...
`Locale` type from `@open-aippt/core`.

```ts title="open-aippt.config.ts"
import type { OpenAipptConfig, Locale } from '@open-aippt/core';
import { en } from '@open-aippt/core/locale';
...
const config: OpenAipptConfig = { locale: fr };
export default config;
```
