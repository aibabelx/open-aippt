# Contributing to open-aippt

Thanks for your interest in improving open-aippt! This guide covers the workflow for contributing to the framework itself — the `@open-aippt/core` runtime, the `@open-aippt/cli` scaffolder, and the supporting apps.
...
- **Ask a question or share what you're building** in [GitHub Discussions](https://github.com/aibabelx/open-aippt/discussions).
...
| Path | Package | Role |
| --- | --- | --- |
| [`packages/core`](packages/core) | `@open-aippt/core` | Runtime (viewer, present mode, inspector), Vite plugin, `open-aippt` dev/build CLI. |
| [`packages/cli`](packages/cli) | `@open-aippt/cli` | `npx @open-aippt/cli init` scaffolder + project template. |
| [`apps/demo`](apps/demo) | private | Local consumer of `@open-aippt/core` via `workspace:*`. The dogfood target for the framework. |
...
```bash
git clone https://github.com/aibabelx/open-aippt.git
cd open-aippt
pnpm install
```

Then run the demo against the local `@open-aippt/core`:
...
Releases are cut by the maintainer via `pnpm release`, which builds `@open-aippt/core` + `@open-aippt/cli` and runs `changeset publish`. Contributors don't need to publish anything — just land the changeset alongside your code.

## Questions

Open a [discussion](https://github.com/aibabelx/open-aippt/discussions) — happy to help.
