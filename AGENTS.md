# open-aippt — Framework Repo Guide

You are working on the **open-aippt framework** — the runtime, CLI, and tooling that ship to npm.
...
| Path | Package | Role |
| --- | --- | --- |
| `packages/core` | `@open-aippt/core` | Runtime (viewer, present mode, inspector), Vite plugin, `open-aippt` dev/build CLI. |
| `packages/cli` | `@open-aippt/cli` | `npx @open-aippt/cli init` scaffolder + project template. |
| `apps/demo` | private | Local consumer of `@open-aippt/core` via `workspace:*`. Dogfood target — run `pnpm dev` here to exercise the framework. |
